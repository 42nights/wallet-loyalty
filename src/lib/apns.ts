...
import { PASS_TYPE_ID } from "./config";

// For Wallet pass updates the push is EMPTY ({}). It just tells the device
// "this pass changed, come re-download it". Authenticated with your Pass Type
// ID signing certificate as a TLS client cert. apns-topic = the pass type id.
...
export async function pushPass(pushToken: string): Promise<boolean> {
...
export async function pushMany(tokens: string[]): Promise<void> {
  await Promise.all(tokens.map((t) => pushPass(t)));
}
